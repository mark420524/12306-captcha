...
# 将synset转换为train.txt文件  修改之,完全没必要
...
def create_train_file(data_path, file_list, train_file_name='image/train.txt'):
    create_file(data_path, file_list, train_file_name)


def create_test_file(data_path, file_list, test_file_name='image/test.txt'):
    create_file(data_path, file_list, test_file_name)


if __name__ == '__main__':
    create_train_file( "E:\\aaaaa\\download_captcha\\traindata\\", read_load_label())
    create_test_file(  "E:\\aaaaa\\download_captcha\\traindata\\", read_load_label() )